...
function getHttpStatusMessage(statusCode: HttpStatusCode): string {
  return HttpStatusMessages[statusCode] || 'Unknown status code';
}

export function getHttpStatus(status: keyof typeof HttpStatusCode) {
  return {
    code: HttpStatusCode[status],
    message: getHttpStatusMessage(HttpStatusCode[status]),
  };
}
